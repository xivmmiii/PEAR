...
export default function BagPage() {
  ...
  async function update(item: Item, quantity: number) {
    if (quantity < 1) return remove(item.productId);
  ...
  async function remove(productId: string) { await fetch(`/api/shopper/bag?productId=${encodeURIComponent(productId)}`, { method: "DELETE" }); void load(); }
  const total = items.reduce((sum, item) => sum + item.product.price * item.quantity, 0);
  return <><SiteHeader /><main className="bag-page page-width"><p className="eyebrow">Your saved good stuff</p><h1>Your <i>bag.</i></h1>{loading ? <p className="empty-inline">Loading your bag...</p> : items.length ? <div className="bag-layout"><div className="bag-items">{items.map((item) => <article className="bag-item" key={item.productId}><Image src={item.product.imageUrl} alt={item.product.name} width={130} height={160} /><div><p className="product-brand">{item.product.brand}</p><h2>{item.product.name}</h2><p>Size: {item.size ?? "Not selected"}</p><strong>₹{item.product.price.toLocaleString("en-IN")}</strong><div className="quantity-controls"><button onClick={() => update(item, item.quantity - 1)}>-</button><span>{item.quantity}</span><button onClick={() => update(item, item.quantity + 1)}>+</button><button className="remove-item" onClick={() => remove(item.productId)}>Remove</button></div></div></article>)}</div><aside className="bag-summary"><p className="eyebrow">Summary</p><div><span>Subtotal</span><strong>₹{total.toLocaleString("en-IN")}</strong></div><div><span>Shipping</span><span>Free over ₹999</span></div><hr /><div className="bag-total"><span>Total</span><strong>₹{total.toLocaleString("en-IN")}</strong></div><Link href="/checkout" className="button button-dark">Continue to checkout <span>↗</span></Link></aside></div> : <div className="empty-state"><h2>Your bag is waiting.</h2><p>Add something good from the catalogue to get started.</p><Link href="/catalogue" className="button button-dark">Explore catalogue <span>↗</span></Link></div>}</main><SiteFooter /></>;
}
